...
def _migrate_mr5m_paper(db: sqlite3.Connection) -> None:
    ...
    db.execute("""
        CREATE TABLE IF NOT EXISTS mr5m_paper (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            open_ts REAL NOT NULL, open_human TEXT, side TEXT, entry REAL, zscore REAL,
            close_ts REAL, exit REAL, pnl_bps REAL, reason TEXT, status TEXT DEFAULT 'OPEN'
        )
    """)
    db.execute("CREATE INDEX IF NOT EXISTS idx_mr5m_ts ON mr5m_paper(open_ts DESC)")


def log_mr5m_open(side: str, entry: float, z: float) -> int:
    from datetime import datetime, timezone
    try:
        with _conn() as db:
            cur = db.execute(
                "INSERT INTO mr5m_paper (open_ts,open_human,side,entry,zscore,status) VALUES (?,?,?,?,?, 'OPEN')",
                (datetime.now().timestamp(), datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S"),
                 str(side), float(entry or 0), float(z or 0)))
            return int(cur.lastrowid or 0)
    except Exception:
...
def log_mr5m_close(rid: int, exit_px: float, pnl_bps: float, reason: str) -> None:
    from datetime import datetime
    try:
        with _conn() as db:
            db.execute("UPDATE mr5m_paper SET close_ts=?, exit=?, pnl_bps=?, reason=?, status='CLOSED' WHERE id=?",
                       (datetime.now().timestamp(), float(exit_px or 0), float(pnl_bps or 0), str(reason), int(rid)))
    except Exception:
        pass


def _migrate_poc_paper(db: sqlite3.Connection) -> None:
    """Strateji D: Hacim Profili / POC reversion paper (shadow) — gercek emir YOK.
    6GB 8-yaklasim testi KAZANANI (net +1935, OOS +1192). zscore kolonu=POC sapmasi(bps)."""
    db.execute("""
        CREATE TABLE IF NOT EXISTS poc_paper (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            open_ts REAL NOT NULL, open_human TEXT, side TEXT, entry REAL, zscore REAL,
            close_ts REAL, exit REAL, pnl_bps REAL, reason TEXT, status TEXT DEFAULT 'OPEN'
        )
    """)
    db.execute("CREATE INDEX IF NOT EXISTS idx_poc_ts ON poc_paper(open_ts DESC)")


def log_poc_open(side: str, entry: float, dev: float) -> int:
    from datetime import datetime, timezone
    try:
        with _conn() as db:
            cur = db.execute(
                "INSERT INTO poc_paper (open_ts,open_human,side,entry,zscore,status) VALUES (?,?,?,?,?, 'OPEN')",
                (datetime.now().timestamp(), datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S"),
                 str(side), float(entry or 0), float(dev or 0)))
            return int(cur.lastrowid or 0)
    except Exception:
        return 0


def log_poc_close(rid: int, exit_px: float, pnl_bps: float, reason: str) -> None:
    from datetime import datetime
    try:
        with _conn() as db:
            db.execute("UPDATE poc_paper SET close_ts=?, exit=?, pnl_bps=?, reason=?, status='CLOSED' WHERE id=?",
                       (datetime.now().timestamp(), float(exit_px or 0), float(pnl_bps or 0), str(reason), int(rid)))
    except Exception:
        pass
