    ...
    if bool(getattr(cfg, "V3_STRATEGY_B_ENABLED", False)):
        try:
            ...
            def _bjx(reason):
                try:
                    from botlog.db import log_d_journal, get_open_trade_id
                    log_d_journal("EXIT", strategy="B", signal=side, price=mark,
                                  reason=f"{reason} kar={cur_bps:+.0f}bps", trade_id=int(get_open_trade_id() or 0))
                except Exception:
                    pass
