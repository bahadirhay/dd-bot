"""scripts/_d_30m_robust.py — 30m D adayini DAHA SAGLAM test et: 180 gun (15m baseline ile ayni
sure) + 3 coin (ETH/BTC/SOL, SMA-align'in daha once cok-coin dogrulandigi liste) + 4-pencere WF.
Amac: tek-coin/tek-pencere 120-gunluk sonucun (+925bps, 3/4) gercekten robust mu yoksa ETH'ye
ozel tek-pencere sansi mi oldugunu ayirt etmek.
"""
...
def kl(sym, interval="30m", sec=1800, days=DAYS):
    out = {}
...
def test_coin(sym):
    ...
    if not trades:
        print(f"{sym:>9}: bar={n:6d}  islem YOK\n")
        return None
    ...
    print(f"{sym:>9}: bar={n:6d} n={len(trades):4d} | {row} | TOPLAM={tot:+8.0f}bps | "
          f"pencere={pos_w}/{n_win} | win%={wins/len(trades)*100:4.0f} | ort={tot/len(trades):+6.1f}bps/islem")
    return tot


def main():
    print(f"D @ 30m, {DAYS} gun, 3-coin robustluk testi "
    ...
    pos = sum(1 for s, r in results if r > 0)
    print(f"\n{pos}/{len(results)} coin pozitif net verdi.")
