"""scripts/_d_entrystop_poctp_wf.py — kullanicinin onerdigi ALTERNATIF exit: TP=POC'a dokunma (min-kar
sarti YOK), SL=giris fiyatinin KENDISI (sabit, fiyat girise geri donerse/gecerse hemen cik, tolerans yok).
MAXHOLD yine backstop olarak kalir. Ayni sma_align/er_trend giris filtreleriyle, MEVCUT-D'ye (ATR-SL +
POC-revert+min-kar12bps) karsi 180-gun/4-pencere WF kiyasi.
"""
...
def main():
    ...
    def sim_exit_baseline(i, entry, sig):
    ...
    def sim_exit_entrystop(i, entry, sig):
        """SL = giris fiyatinin KENDISI (sabit). TP = POC'a DOKUNMA (min-kar sarti yok)."""
        sl = entry   # SIFIR-toleranslı: fiyat girise geri donerse/gecerse hemen cik
        exit_px = None; reason = "maxhold"; jend = i
        for j in range(i + 1, min(i + MAXHOLD + 1, n)):
            jend = j
            if sig == "LONG" and L[j] <= sl:
                exit_px = sl; reason = "sl_entry"; break
            if sig == "SHORT" and H[j] >= sl:
                exit_px = sl; reason = "sl_entry"; break
            pocj = poc_at(j)
            if pocj > 0:
                ...
                reverted = (devj >= 0) if sig == "LONG" else (devj <= 0)
                if reverted:
                    exit_px = C[j]; reason = "poc_touch"; break
        if exit_px is None:
            exit_px = C[jend]; reason = "maxhold"
        pnl = ((exit_px - entry) if sig == "LONG" else (entry - exit_px)) / entry * 1e4 - FEE
        return pnl, reason, jend

    def gen_trades(exit_fn):
        out = []
        ...
        while i < n:
            ...
            if blocked:
                i += 1; continue
            pnl, reason, jend = exit_fn(i, px, sig)
            out.append((T[i], pnl, reason))
            i = jend + 1
        return out

    for label, exit_fn in (("MEVCUT-D (ATR-SL + POC-revert+min-kar12)", sim_exit_baseline),
                            ("ONERI (SL=giris-fiyati + POC-dokunma)", sim_exit_entrystop)):
        trades = gen_trades(exit_fn)
        if not trades:
            print(f"{label}: islem yok\n"); continue
        ...
        print(f"  n={len(trades)} | {row} | TOPLAM={tot:+.0f}bps | pencere={pos_w}/{n_win} | "
              f"win%={wins/len(trades)*100:.0f} | ort={tot/len(trades):+.0f}bps/islem | sebepler={reasons}\n")
